...
def fig_observe_simule(obs, sim, treatments=['T1','T2'], 
    ...
    for ifig, treatment in enumerate(treatments):
        ...
        if add_sim is not None:
            obs_bar, sim_bar = barplot_leaf(ax, add_obs, add_sim, treatment,xleaf=xleaf,
                                prefix=prefix, s_color=s_color)
        ...
        if ylim is not None:
            ax.set_ylim(ylim)
        ax.text(min(xleaf) + 0.5, .8 * max(ax.get_ylim()), ''+str(treatment), bbox={'facecolor':'#FCF8F8', 'alpha':0.6, 'pad':10}, fontsize=12)
    fig.tight_layout() 
...
def deposit_observe_simule(variety = 'Tremie12', nplants = 30, nrep = 1, axis='MS', by='ntop_cur', xleaf=None, ylim=(0,6.5), 
    ...
    fig = fig_observe_simule(obs, sim, treatments=treatments, xleaf=xleaf, prefix=prefix, s_color=color, ylim=ylim, ylab=ylab,title=title)
    return fig
...
def hs_observe_simule(variety ='Tremie12', nplants = 30, nrep = 1, axis='MS',
    ...
    sim = ifun.axis_statistics(df_sim, what='haun_stage', axis=axis)
    ...
    for t in treatments:
        ...
        #xbar += bar_width
        if t in obs.index:
            obs_bar = ax.bar(xbar, obs.ix[t,'HS_mean'], bar_width, alpha=opacity, color='y', yerr=obs.ix[t,'HS_conf'], ecolor='y')
        xbar += bar_width
        xstage.append(xbar)
        sim_bars = ax.bar(xbar, sim.ix[t,'ybar'], bar_width, alpha=opacity, color=colors[var], yerr=sim.ix[t,'yerr'], ecolor=colors[var])
        xbar += bar_width * 1.5
    ax.set_ylim(0, 14)
    ax.set_xlim(-bar_width, xbar)                 
    ax.set_xticks(xstage)
    ax.set_xticklabels(treatments, fontsize='small' )
    ax.text(0.1, 13.1, ''+str(var), bbox={'facecolor':'#FCF8F8', 'alpha':0.6, 'pad':10}, fontsize=12)
    fig.text(0.5, 0.05, 'HS', size='large', ha='center')
    return fig, ax

def scan_observe_simule(variety = 'Tremie12', nplants = 30, nrep = 1, axis='MS', by='metamer',xleaf=None, ylim=(0,30),
                         simulation='reference', treatments=None, reset=False, reset_data=False):
    if treatments is None:
    ...
    add_obs = ifun.leaf_statistics(df_obs.loc[variety,:], what='A_bl_green', by=by, axis=axis)
    #sim
    ...
    add_sim = ifun.leaf_statistics(df_sim, what='green_area', by=by, axis=axis)
    #plot
    color = colors_variety()[variety]
    prefix = prefix_xbar()[by]
    if xleaf is None:
    ...
    ylab = 'Leaf Area (cm2)'
    title = variety + ' ' + simulation
    fig = fig_observe_simule(obs, sim, treatments=treatments, xleaf=xleaf, prefix=prefix, s_color=color, ylim=ylim, ylab=ylab,title=title,add_obs=add_obs,add_sim=add_sim)
    return fig
    
def sil_observe_simule(variety = 'Tremie12', nplants = 30, nrep = 1, axis='MS', by='ntop_cur', xleaf=None, ylim=(0,1), 
                what='h_projection', simulation='reference', treatments=None, reset=False, reset_data=False):
    if treatments is None:
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS'], df_sim_ntop['deposit_Tartrazine'], picto+color, label=label)
            #fleche hsT2
            date, hsT2 = HS_applications[var]['T2']
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS-nffmean'], df_sim_ntop['deposit_Tartrazine'], picto+color, label=label)
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS'], df_sim_ntop['deposit_Tartrazine/area'], picto+color, label=label)
            #fleche hsT2
            date, hsT2 = HS_applications[var]['T2']
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS-nffmean'], df_sim_ntop['deposit_Tartrazine/area'], picto+color, label=label)
            #fleche hsT2 - nff moyen
            hs_new = hsT2 - hs_moyen
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS'], df_sim_ntop['coverage_efficacy'], picto+color, label=label)
            #fleche hsT2
            date, hsT2 = HS_applications[var]['T2']
            plt.annotate('', xy=(hsT2, 0), xytext=(hsT2, -0.7), arrowprops=dict(color=color, arrowstyle="->", connectionstyle="arc3"))
        #mise en forme
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS-nffmean'], df_sim_ntop['coverage_efficacy'], picto+color, label=label)
            #fleche hsT2 - nff moyen
            hs_new = hsT2 - hs_moyen
            plt.annotate('', xy=(hs_new, 0), xytext=(hs_new, -0.03), arrowprops=dict(color=color, arrowstyle="->", connectionstyle="arc3"))
        #mise en forme
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS'], df_sim_ntop['protection_efficacy'], picto+color, label=label)
            #fleche hsT2
            date, hsT2 = HS_applications[var]['T2']
            plt.annotate('', xy=(hsT2, 0), xytext=(hsT2, -0.7), arrowprops=dict(color=color, arrowstyle="->", connectionstyle="arc3"))
        #mise en forme
        ...
        for var in varieties:        
            ...
            for ntop in ntop_lst:
                ...
                if ntop==1:
                    ...
                    picto=':'
                #plot par ntop
                plt.plot(df_sim_ntop['HS-nffmean'], df_sim_ntop['protection_efficacy'], picto+color, label=label)
            #fleche hsT2 - nff moyen
            hs_new = hsT2 - hs_moyen
            plt.annotate('', xy=(hs_new, 0), xytext=(hs_new, -0.03), arrowprops=dict(color=color, arrowstyle="->", connectionstyle="arc3"))
        #mise en forme
